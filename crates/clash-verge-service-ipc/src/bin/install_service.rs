...
fn resolve_service_group_name() -> String {
    ...
    if let Some(gid) = env_u32("MUACLOUD_SERVICE_GID")
...
fn main() -> Result<(), Error> {
    ...
    let service_binary_path = env::current_exe().unwrap().with_file_name("muacloud-service");

    if !service_binary_path.exists() {
        return Err(anyhow::anyhow!("muacloud-service binary not found"));
    }

    // 定义 bundle 路径
    let bundle_path = "/Library/PrivilegedHelperTools/com.muacloud.client.service.bundle";
    ...
    let target_binary_path = format!("{}/muacloud-service", macos_path);
    ...
    // 创建并写入 launchd plist
    let plist_file = "/Library/LaunchDaemons/com.muacloud.client.service.plist";
    ...
    // 加载和启动服务
    let _ = run_command("launchctl", &["enable", "system/com.muacloud.client.service"], debug);
    ...
    let _ = run_command("launchctl", &["start", "com.muacloud.client.service"], debug);
...
#[cfg(target_os = "linux")]
fn main() -> Result<(), Error> {
    const SERVICE_NAME: &str = "muacloud-service";
    use std::env;
    use std::fs::File;
    use std::io::Write;
    use std::path::Path;
...
fn main() -> anyhow::Result<()> {
    ...
    if let Ok(service) = service_manager.open_service("muacloud_service", service_access)
    ...
    let service_binary_path = env::current_exe().unwrap().with_file_name("muacloud-service.exe");

    if !service_binary_path.exists() {
        eprintln!("muacloud-service.exe not found");
        std::process::exit(2);
    }

    let service_info = ServiceInfo {
        name: OsString::from("muacloud_service"),
        display_name: OsString::from("MuaCloud Service"),
    ...
    service.set_description("MuaCloud Service helps to launch mihomo core")?;
    service.start(&Vec::<&OsStr>::new())?;
