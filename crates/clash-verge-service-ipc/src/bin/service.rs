//! MuaCloud Service - Cross-platform IPC service daemon
//!
...
fn main() -> Result<()> {
    init_logger();
    if service_dispatcher::start("muacloud_service", ffi_service_main).is_err() {
...
fn run_service() -> platform_lib::Result<()> {
    ...
    let status_handle = service_control_handler::register("muacloud_service", event_handler)?;
...
async fn run_standalone() -> Result<()> {
    ...
    info!("MuaCloud Service - Standalone Mode");
